...
def get_cluster_recommendations_from_referrals(services, n_recommendations=5):
    cluster_recommendations = model.execute(
        queries.get_cluster_recommendations_from_clusters, (services, n_recommendations,))
    cluster_ids = [row['cluster_id'] for row in cluster_recommendations]
    return cluster_ids
